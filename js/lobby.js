...
function initLobbyTabs() {
  ...
  setLobbyTab(['settings', 'lobby', 'chat', 'more'].includes(savedTab) ? savedTab : 'lobby');
}

function setLobbyTab(tab = 'lobby') {
  const activeTab = ['settings', 'lobby', 'chat', 'more'].includes(tab) ? tab : 'lobby';
  const body = document.querySelector('.lv2-body');
  if (body) body.dataset.activeTab = activeTab;
