...
const LOBBY_EFFECTS_KEY = 'nightmares_lobby_low_effects';
const LOBBY_TABS = ['chat', 'players', 'lobby', 'settings', 'more'];
const LOBBY_TAB_LABELS = {
  chat: 'الدردشة',
  players: 'اللاعبون',
  lobby: 'اللوبي',
  settings: 'الإعدادات',
...
// ── Lobby tabs / safe panels ─────────────────────────────────────────────────
function initLobbyTabs() {
  if (lobbyTabsBound) return;
  lobbyTabsBound = true;

  document.querySelectorAll('[data-lobby-tab]').forEach(btn => {
    btn.addEventListener('click', () => setLobbyTab(btn.dataset.lobbyTab || 'lobby'));
  });

  const savedTab = localStorage.getItem('nightmares_lobby_tab') || 'lobby';
  setLobbyTab(LOBBY_TABS.includes(savedTab) ? savedTab : 'lobby');
}

function setLobbyTab(tab = 'lobby') {
  const activeTab = LOBBY_TABS.includes(tab) ? tab : 'lobby';
  const lobby = document.getElementById('lobby-page');
  if (lobby) lobby.dataset.activeTab = activeTab;
  document.body.dataset.lobbyTab = activeTab;

  document.querySelectorAll('[data-lobby-tab]').forEach(btn => {
    const isActive = btn.dataset.lobbyTab === activeTab;
    btn.classList.toggle('active', isActive);
    btn.setAttribute('aria-pressed', String(isActive));
  });

  const label = document.getElementById('v3-section-label');
  if (label) label.textContent = LOBBY_TAB_LABELS[activeTab] || LOBBY_TAB_LABELS.lobby;

  const panelTitle = document.querySelector('#host-settings .lv2-panel-title');
  if (panelTitle) panelTitle.textContent = activeTab === 'more' ? 'المزيد' : 'الإعدادات';

  localStorage.setItem('nightmares_lobby_tab', activeTab);
}

function getSavedLobbyVisualMode() {
  return localStorage.getItem(LOBBY_THEME_KEY) === 'light' ? 'clear' : 'dark';
}

function storeLobbyVisualMode(mode = 'dark') {
  localStorage.setItem(LOBBY_THEME_KEY, mode === 'clear' ? 'light' : 'dark');
}

function initLobbyPersonalSettings() {
  syncVisualModeButtons(getSavedLobbyVisualMode());
  applyPersonalEffectsSetting();

  const soundBtn = document.getElementById('lv2-personal-sound');
  if (soundBtn) {
    const syncSoundText = () => {
      soundBtn.textContent = Sound?.isEnabled?.() ? 'تشغيل' : 'إيقاف';
    };
    syncSoundText();
  ...
  document.getElementById('lv2-personal-effects')?.addEventListener('click', () => {
    const nextLowEffects = localStorage.getItem(LOBBY_EFFECTS_KEY) !== 'true';
    localStorage.setItem(LOBBY_EFFECTS_KEY, nextLowEffects ? 'true' : 'false');
    applyPersonalEffectsSetting();
  });
}

function applyPersonalEffectsSetting() {
  const lowEffects = localStorage.getItem(LOBBY_EFFECTS_KEY) === 'true';
  document.body.classList.toggle('lv2-low-effects', lowEffects);
  const btn = document.getElementById('lv2-personal-effects');
  if (btn) btn.textContent = lowEffects ? 'إيقاف' : 'تشغيل';
  if (lowEffects) {
    try { Effects?.stopAllEffects?.(); } catch (e) {}
  }
}

function syncLobbyPermissions(me) {
  const isHost = !!me?.isHost;
  const settingInputs = ['s-maxPlayers','s-nightTime','s-dayTime','s-discussTime','s-votingTime','s-defenseTime'];
  settingInputs.forEach(id => {
    const input = document.getElementById(id);
    if (input) input.disabled = !isHost;
  });
  document.querySelectorAll('.sr-toggle, .er-toggle, #btn-enable-all, #btn-disable-all').forEach(el => {
    el.disabled = !isHost;
  });
  document.getElementById('host-settings')?.classList.toggle('lv2-readonly-panel', !isHost);
  const hint = document.getElementById('v3-settings-host-hint');
  if (hint) hint.textContent = isHost ? 'أنت الهوست — التعديلات تحفظ مباشرة' : 'الهوست فقط يستطيع تعديل إعدادات الغرفة';
}
...
function onRoomUpdate(data) {
  ...
  const activePlayers = getLobbyActivePlayers(data.players || {});
  updateLobbyWaitingBox(activePlayers.length, LOBBY_MIN_PLAYERS, activePlayers.length >= LOBBY_MIN_PLAYERS);
  renderEnabledExtraRoles(data.settings?.enabledExtraRoles || {});
  updateStartButton(data);
  updateReadyButton(me.ready);
  syncVisualModeButtons(getSavedLobbyVisualMode());
  syncLobbyPermissions(me);
  ...
  if (mobSettings) mobSettings.style.display = 'none';
  if (mobStartBar) mobStartBar.style.display = 'none';

  // Host settings display
  if (data.settings) {
    document.getElementById('s-maxPlayers').value = getLobbyMaxPlayers(data.settings);
...
function getLobbyActivePlayers(playersMap = {}) {
  return Object.values(playersMap || {}).filter(p => p && p.id && isPlayerActive(p));
}

function getLobbyMaxPlayers(settings = {}) {
  const rawMax = parseInt(settings.maxPlayers, 10);
  if (!Number.isFinite(rawMax)) return 6;
  return Math.min(12, Math.max(LOBBY_MIN_PLAYERS, rawMax));
}

function updateLobbySummary(data) {
  const players = getLobbyActivePlayers(data.players || {});
  const maxPlayers = getLobbyMaxPlayers(data.settings || {});
  const countText = `${players.length} / ${maxPlayers}`;
  const compactText = `${players.length}/${maxPlayers}`;
  const readyCount = players.filter(p => p.isHost || p.ready).length;
  const heroCount = document.getElementById('v3-hero-player-count');
  const playerCount = document.getElementById('v3-player-count');
  const statusPlayerCount = document.getElementById('v3-status-player-count');
  const statusReadyCount = document.getElementById('v3-status-ready-count');
  if (heroCount) heroCount.textContent = countText;
  if (playerCount) playerCount.textContent = compactText;
  if (statusPlayerCount) statusPlayerCount.textContent = compactText;
  if (statusReadyCount) statusReadyCount.textContent = `${readyCount}/${players.length || maxPlayers}`;
  const host = Object.values(data.players || {}).find(p => p.isHost);
  document.body.dataset.viewerHost = data.players?.[session.id]?.isHost ? 'true' : 'false';
  document.body.dataset.hostName = host?.name || '';
}
...
function renderPlayers(players) {
  ...
  const sortedPlayers = Object.values(players || {}).filter(p => p && p.id).sort((a, b) => {
  ...
  const activePlayers = sortedPlayers.filter(p => isPlayerActive(p));
  const maxPlayers = getLobbyMaxPlayers(roomData?.settings || {});
  const slotCount = Math.max(maxPlayers, activePlayers.length);
  ...
  for (let i = 0; i < slotCount; i++) {
    ...
    const slot = player
      ? createLobbyPlayerRow(player, slotNumber, viewerIsHost, true)
      : createEmptyLobbySlot(slotNumber);

    slot.style.gridColumn = slotNumber <= leftCount ? '1' : '2';
    slot.style.gridRow = slotNumber <= leftCount ? String(slotNumber) : String(slotNumber - leftCount);
    slot.classList.add(slotNumber <= leftCount ? 'v3-slot-left' : 'v3-slot-right');
    list.appendChild(slot);
  }
  bindLobbyKickButtons(list);

  const detailList = document.getElementById('players-detail-list');
  if (detailList) {
    detailList.innerHTML = '';
    sortedPlayers.forEach((p, index) => {
      detailList.appendChild(createLobbyPlayerRow(p, index + 1, viewerIsHost, false));
    });
    bindLobbyKickButtons(detailList);
  }
}

function createLobbyPlayerRow(p, index, viewerIsHost, isSlot = false) {
  const status = getPlayerStatus(p);
  const inactive = !isPlayerActive(p);
  const points = typeof p.points === 'number' ? p.points : 0;
  const pointsBadge = `<span class="player-points-badge${points >= 3 ? ' player-points-badge-hot' : ''}">★ ${points}</span>`;
  const canKick = viewerIsHost && p.id !== session.id && !p.isHost && status !== 'kicked' && status !== 'left';
  const div = document.createElement('div');
  div.className = `player-list-item player-status-${status}${isSlot ? ' v3-lobby-slot' : ' v3-detail-player-row'}${points >= 3 ? ' player-points-hot' : ''}`;
  div.innerHTML = `
    <span class="player-rank-badge">${index}</span>
...
function createEmptyLobbySlot(slotNumber) {
  const div = document.createElement('div');
  div.className = 'player-list-item v3-lobby-slot v3-slot-empty';
  div.setAttribute('aria-label', `مقعد فارغ ${slotNumber}`);
  div.innerHTML = `
    <span class="player-rank-badge">${slotNumber}</span>
    <span class="player-list-icon"><span class="v3-empty-avatar" aria-hidden="true"></span></span>
    <span class="player-list-name"><span class="v3-empty-name-line" aria-hidden="true"></span></span>
    <span class="player-list-badge badge-waiting" aria-hidden="true"></span>
  `;
  return div;
}

function bindLobbyKickButtons(container) {
  container.querySelectorAll('.lobby-kick-btn').forEach(kickBtn => {
...
function updateReadyButton(ready) {
  ...
  if (ready) {
    ...
    btn.className = 'v3-ready-button is-ready';
  } else {
    btn.textContent = 'جاهز';
    btn.className = 'v3-ready-button';
  }
}
...
function updateLobbyWaitingBox(count, minPlayers, enoughPlayers) {
  const waitingBox = document.getElementById('v3-waiting-box');
  const readyBtn = document.getElementById('btn-ready');
  const startBtn = document.getElementById('btn-start');
  if (!waitingBox) return;
  if (count < minPlayers) {
    waitingBox.hidden = false;
    waitingBox.textContent = `يلزم ${minPlayers} لاعبين على الأقل (${count}/${minPlayers})`;
    if (readyBtn) readyBtn.hidden = true;
    if (startBtn) startBtn.hidden = true;
    return;
  }
  waitingBox.hidden = true;
  waitingBox.textContent = '';
...
function checkHostControls() {
  ...
  document.getElementById('btn-enable-all')?.addEventListener('click', () => {
    document.querySelectorAll('.sr-toggle, .er-toggle').forEach(cb => cb.checked = true);
    pushSettings();
  });

  document.getElementById('btn-disable-all')?.addEventListener('click', () => {
    document.querySelectorAll('.sr-toggle, .er-toggle').forEach(cb => cb.checked = false);
    pushSettings();
  });
}
...
function syncVisualModeButtons(mode = 'dark') {
  ...
  document.body.classList.toggle('lv2-light', visualMode === 'clear');
  document.body.classList.toggle('lv2-dark', visualMode !== 'clear');
  storeLobbyVisualMode(visualMode);
...
async function startGame() {
  ...
  players.forEach(p => {
    ...
    updates[`rooms/${session.roomId}/players/${p.id}/phoenixChosenTeam`] = null;
  ...
  updates[`rooms/${session.roomId}/game/phoenixRevives`] = {};
...
function initChat() {
  ...
  if (window.__lobbyChatInitializedForRoom === session.roomId) return;
  window.__lobbyChatInitializedForRoom = session.roomId;
...
function formatLobbyChatTime(ts) {
  if (!ts) return '';
  try {
...
function renderChat(chatData) {
  ...
  msgs.forEach(msg => {
    const div = document.createElement('div');
    const isMine = msg.playerId === session.id;
    const isSystem = msg.playerId === 'system' || msg.type === 'system';
    div.className = `chat-msg${isMine ? ' is-me' : ''}${isSystem ? ' is-system' : ''}`;
    div.innerHTML = `
      <span class="icon">${getPlayerPortraitHtml(msg.icon, msg.name, 'chat-portrait')}</span>
      <span class="name">${escapeHtml(msg.name || 'النظام')}</span>
      <span class="time">${formatLobbyChatTime(msg.ts)}</span>
