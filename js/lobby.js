...
function renderPlayers(players) {
  ...
  Object.values(players).forEach(p => {
    ...
    div.className = `player-list-item player-status-${status}${p.id === session.id ? ' current-player' : ''}`;
    div.innerHTML = `
      <span class="player-list-icon">${renderAvatarHtml(p, 'avatar-list')}</span>
      <span class="player-list-name">${escapeHtml(p.name)}${p.id === session.id ? ' (أنت)' : ''}</span>
      ${inactive
        ? `<span class="player-list-badge badge-left">${status === 'kicked' ? 'مطرود' : status === 'offline' ? 'غير متصل' : 'غادر'}</span>`
        : p.isHost
...
function renderPlayerSeats(players) {
  ...
  pArr.forEach((p, i) => {
    ...
    seat.className = `player-seat player-status-${status}${p.ready || p.isHost ? ' ready' : ''}${p.isHost ? ' host' : ''}${p.id === session.id ? ' current-player' : ''}`;
    seat.style.left = pos.x + '%';
    seat.style.top = pos.y + '%';
    seat.innerHTML = `
      <div class="player-seat-icon">${renderAvatarHtml(p, 'avatar-seat')}</div>
      <div class="player-seat-name">${escapeHtml(p.name)}${p.isHost ? ' 👑' : ''}</div>
      <div class="player-seat-status">${!isPlayerActive(p) ? (status === 'kicked' ? 'مطرود' : status === 'offline' ? 'غير متصل' : 'غادر') : p.ready || p.isHost ? '✓ جاهز' : 'ينتظر...'}</div>
    `;
...
async function pushSettings() {
  if (!roomData) return;
  const me = roomData.players?.[session.id];
  if (!me?.isHost || !isPlayerActive(me)) return;
...
async function startGame() {
  ...
  players.forEach(p => {
    ...
    updates[`rooms/${session.roomId}/players/${p.id}/status`] = 'online';
