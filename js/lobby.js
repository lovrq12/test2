...
async function startGame() {
  ...
  // Surface founder warning if any
  if (roleAssignment._founderWarning) {
    showToast(roleAssignment._founderWarning, 'error', 6000);
  }
  ...
  players.forEach(p => {
    ...
    if (!roleId || roleId.startsWith('_')) return; // skip meta keys
  ...
  // Identify mafia leader: Cursed is always leader (also flagged in ROLES), else first mafia
  const mafiaPlayers = players.filter(p => RoleEngine.isMafia(roleAssignment[p.id]));
  const cursedPlayer = mafiaPlayers.find(p => roleAssignment[p.id] === 'cursed');
  const leader = cursedPlayer || mafiaPlayers[0];
  if (leader) {
    updates[`rooms/${session.roomId}/players/${leader.id}/isMafiaLeader`] = true;
  }
