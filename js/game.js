    ...
    pArr.forEach((p, i) => {
      ...
      seat.innerHTML = `
        ...
        <span class="ptc-frame">
          <span class="ptc-animal-icon" role="img" aria-label="${escapeHtml(p.name)}">${getAnimalEmoji(p.icon)}</span>
          ...
          ${iAmMafia && targetIsMafia && p.id !== session.id ? '<span class="ptc-private-badge ptc-mafia-badge" title="زميل مافيا">⚔</span>' : ''}
...
function getRoleDisplayData(me = {}) {
  ...
  return {
    roleData,
    roleName,
    image: isLiar ? getLiarDisplayCard(me) : roleData.image,
...
function appendWhisperAbility(list, game, me, players) {
  ...
  label.textContent = 'الهمسة: اكشف بطاقة لاعب حي لنفسك فقط لمدة 10 ثوانٍ';
...
function initChatTabs() {
  // CHAT UI REMOVED — function preserved to prevent call-site errors
  return;
  ...
  try {
    ...
    Object.entries(players).forEach(([playerId, player]) => {
      ...
      const isWinner =
        ...
        (game.winner === 'citizens' && !RoleEngine.isPlayerMafia(p)) ||
...
function getWinCinematicPlayers(winner, players = {}) {
  return Object.entries(players)
    ...
    .filter(player => {
      ...
      if (winner === 'citizens') return !RoleEngine.isPlayerMafia(player);
      return false;
    });
}
