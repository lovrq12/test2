...
function isPhoenixRevivePending(player = {}) {
  return player.role === 'phoenix'
    ...
    && !player.alive
    && !player.usedAbilities?.phoenix_used;
}
...
async function usePhoenixSelfRevive(chosenTeam) {
  ...
  await db.ref().update({
    ...
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/phoenix_used`]: true,
...
function handlePlayerTargetClick(targetPlayerId, anchorEl = null) {
  ...
  if (!targetIsAlive && !canVictimRevive) return;
...
function renderPhoenixReviveChoice(game, me) {
  ...
  content.innerHTML = `
    ...
    <button class="btn btn-danger btn-full phoenix-revive-choice" data-team="mafia">انضم للمافيا</button>
    <button class="btn btn-primary btn-full phoenix-revive-choice" data-team="citizens">انضم للمواطنين</button>
  `;
...
function maybePromptPhoenixSelfRevive(game, me) {
  ...
  showActionMenu('عودة العنقاء', [
    { label: 'انضم للمافيا', fn: () => usePhoenixSelfRevive('mafia') },
    { label: 'انضم للمواطنين', fn: () => usePhoenixSelfRevive('citizens') },
  ]);
}
...
function renderDayAbilities(game, me, players) {
  ...
  } else if (false && me.role === 'phoenix' && !me.usedAbilities?.phoenix_used) {
    section.style.display = '';
