...
let activeSafePanel = 'table';
let safeDockContentMounted = false;
...
function ensureInitialGameAssetsLoaded(game = {}, me = {}) {
  ...
  const activeTableBg = isNight
    ? 'assets/backgrounds/table_night_v2.png'
    : 'assets/backgrounds/table_day_v2.png';
...
function setSafeDockPanel(panel = 'table') {
  mountSafeDockContent();
  const allowed = ['table', 'role', 'chat', 'mafia'];
  const requestedPanel = allowed.includes(panel) ? panel : 'table';
  const me = roomData?.players?.[session?.id];
  const canSeeMafia = !!(me && isPlayerActive(me) && RoleEngine.isPlayerMafia(me));
  const nextPanel = requestedPanel === 'mafia' && !canSeeMafia ? 'table' : requestedPanel;
  const layer = document.getElementById('safe-panel-layer');

  activeSafePanel = nextPanel;
  document.body.dataset.safePanel = nextPanel;

  document.querySelectorAll('.safe-dock-tab').forEach(btn => {
    const active = btn.dataset.safePanel === nextPanel;
    btn.classList.toggle('active', active);
    btn.setAttribute('aria-selected', active ? 'true' : 'false');
  });

  document.querySelectorAll('.safe-panel').forEach(panelEl => {
    const active = nextPanel !== 'table' && panelEl.dataset.safePanelContent === nextPanel;
    panelEl.classList.toggle('hidden', !active);
    panelEl.setAttribute('aria-hidden', active ? 'false' : 'true');
  });

  if (layer) layer.classList.toggle('hidden', nextPanel === 'table');
  if (nextPanel === 'chat' || nextPanel === 'mafia') {
    moveChatToSafePanel(nextPanel);
  }
}

function mountSafeDockContent() {
  if (safeDockContentMounted) return;
  const rolePanel = document.getElementById('safe-panel-role');
  const actionPanel = document.querySelector('.action-panel');
  const chatPanel = document.getElementById('safe-panel-chat');
  const chatCard = document.querySelector('.v2-chat-card');

  if (rolePanel && actionPanel && !rolePanel.contains(actionPanel)) {
    rolePanel.appendChild(actionPanel);
    actionPanel.classList.add('safe-dock-role-content');
  }

  if (chatPanel && chatCard && !chatPanel.contains(chatCard)) {
    chatPanel.appendChild(chatCard);
    chatCard.classList.add('safe-dock-chat-content');
  }

  safeDockContentMounted = true;
}

function moveChatToSafePanel(panel) {
  const chatCard = document.querySelector('.v2-chat-card');
  const targetPanel = document.getElementById(panel === 'mafia' ? 'safe-panel-mafia' : 'safe-panel-chat');
  if (!chatCard || !targetPanel) return;
  if (!targetPanel.contains(chatCard)) targetPanel.appendChild(chatCard);

  chatTab = panel === 'mafia' ? 'mafia' : 'public';
  updateChatTabs();
  updateChatAvailability();
  renderPublicChat(latestPublicChatData, latestMafiaChatData);
}

function updateSafeDockVisibility(me = roomData?.players?.[session?.id]) {
  const mafiaTab = document.getElementById('safe-dock-mafia');
  if (!mafiaTab) return;

  const canSeeMafia = !!(me && isPlayerActive(me) && RoleEngine.isPlayerMafia(me));
  mafiaTab.hidden = !canSeeMafia;
  if (!canSeeMafia && activeSafePanel === 'mafia') {
    setSafeDockPanel('table');
  }
}

function initSafeBottomDock() {
  if (window.__safeBottomDockInitialized) return;
  window.__safeBottomDockInitialized = true;
  document.body.classList.add('step1-safe-dock');
  document.querySelectorAll('.safe-dock-tab').forEach(btn => {
    bindTouchSafeAction(btn, () => setSafeDockPanel(btn.dataset.safePanel));
  });
  setSafeDockPanel('table');
}
...
function resolveNightActions(gameOrActions, players) {
  ...
  Object.entries(game.poisoned || {}).forEach(([targetId, poison]) => {
    ...
    const remaining = Number(poison.nightsRemaining || 1);
    if (remaining <= 1) {
...
async function useGovernorReveal(targetPlayerId) {
  ...
  if (game.phase !== 'night' || me?.role !== 'governor' || me.usedAbilities?.governorRounds?.[roundKey]) return;
  if (!isActiveAlive(target) || targetPlayerId === session.id) return;
  ...
  await db.ref().update({
    ...
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/governorRounds/${roundKey}`]: true,
  });
...
function handlePlayerTargetClick(targetPlayerId, anchorEl = null) {
  ...
  if (phase === 'night' && me.role === 'governor' && targetIsAlive && !targetIsSelf) {
    const roundKey = String(game.round || 1);
    const used = me.usedAbilities?.governorRounds?.[roundKey];
    actions.push({
      label: used ? 'تم الكشف هذه الليلة' : 'كشف علني',
      disabled: !!used,
...
const DEFAULT_PUBLIC_TABLE_CARD = 'assets/role_cards/nightmares_public_table_card.png';

function getGovernorPublicReveal(game, playerId) {
  return game?.publicRevealedRoles?.[playerId] || null;
}

function getTableCardImage(game, player) {
  const reveal = getGovernorPublicReveal(game, player?.id);
  if (!reveal) return DEFAULT_PUBLIC_TABLE_CARD;
  return reveal.cardImage || RoleEngine.getRoleImage(reveal.role);
}

function getTableCardAlt(game, player) {
  const reveal = getGovernorPublicReveal(game, player?.id);
  return reveal?.role
    ? RoleEngine.getRoleArabicName(reveal.role)
    : 'كرت لاعب مخفي';
}
...
function renderGameTable(players, game) {
...

  {
    ...
    pArr.forEach((p, i) => {
      ...
      const governorReveal = getGovernorPublicReveal(game, p.id);
      const tableCardImage = getTableCardImage(game, p);
      const tableCardAlt = getTableCardAlt(game, p);
      ...
      seat.className = [
        ...
        governorReveal ? 'public-role-revealed' : '',
      ].filter(Boolean).join(' ');
      ...
      seat.innerHTML = `
        ...
        <span class="ptc-frame">
          ...
          ${!p.alive ? '<span class="ptc-dead-overlay" aria-label="ميت">&#9760;</span>' : ''}
          ...
          ${sharedReveal ? `<span class="ptc-whisper-reveal-badge ptc-reveal-badge" title="${escapeHtml(sharedRevealTitle)}">🔍 مكشوف</span>` : ''}
        </span>
...
function getRoleAbilityText(me = {}, roleData = {}) {
  ...
  const extraAbilityText = {
    ...
    poisoner: 'ليلًا: سمّم لاعبًا من غير المافيا ليموت بعد ليلة واحدة. مرة واحدة.',
...
function getWinCinematicPlayers(winner, players = {}) {
  return Object.entries(players)
    ...
    .filter(player => {
      ...
      if (status === 'kicked' || status === 'left' || !isPlayerActive(player)) return false;
