...
function getRoleTimingText(me = {}, game = roomData?.game || {}) {
  const role = me.role;
  if (['mafia', 'whisper', 'doctor', 'detective', 'poisoner', 'oathbreaker', 'mad_citizen', 'governor', 'victim'].includes(role)) {
    return 'الليل';
  }
...
function getRoleAbilityStatusText(me = {}, game = roomData?.game || {}) {
  ...
  if (['mafia', 'whisper', 'doctor', 'detective', 'poisoner', 'oathbreaker', 'mad_citizen', 'governor', 'victim'].includes(me.role) || RoleEngine.isPlayerMafia(me)) {
    return phase === 'night' ? 'متاحة الآن' : 'تنتظر الليل';
  }
