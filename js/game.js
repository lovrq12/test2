...
(async () => {
  ...
  // ===================================================================
  // DEBUG — camera keyboard controller + live position readout
  // ===================================================================
  _initDebugControls();

  function _initDebugControls() {
    // Show anchors in panel
    const anchEl = document.getElementById('dbg-anchors');
    if (anchEl && Scene3D.ANCHORS) {
      const A = Scene3D.ANCHORS;
      anchEl.innerHTML = [
        `table: (${A.tableCenter.x.toFixed(2)}, ${A.tableCenter.y.toFixed(2)}, ${A.tableCenter.z.toFixed(2)})`,
        `stool: (${A.stoolSeat.x.toFixed(2)}, ${A.stoolSeat.y.toFixed(2)}, ${A.stoolSeat.z.toFixed(2)})`,
        `chair1: (${A.chair1Seat.x.toFixed(2)}, ${A.chair1Seat.y.toFixed(2)}, ${A.chair1Seat.z.toFixed(2)})`,
      ].join('<br>');
    }

    // Live camera position readout
    const camEl = document.getElementById('dbg-campos');
    setInterval(() => {
      const cam = Scene3D.camera;
      if (!cam || !camEl) return;
      const p = cam.position;
      camEl.textContent = `pos: (${p.x.toFixed(2)}, ${p.y.toFixed(2)}, ${p.z.toFixed(2)})`;
    }, 200);

    // WASD + QE camera movement (debug only)
    const keys = {};
    ...
    setInterval(() => {
      const cam = Scene3D.camera;
      if (!cam) return;
      const fast = keys['shift'] ? 3 : 1;
      const s = SPEED * fast;
      if (keys['w']) cam.position.z -= s;
      if (keys['s']) cam.position.z += s;
      if (keys['a']) cam.position.x -= s;
      if (keys['d']) cam.position.x += s;
