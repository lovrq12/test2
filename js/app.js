...
const NEW_EXTRA_ROLE_META = {
  ...
  mad_citizen: {
    ...
    color: '#f97316', abilities: ['mad_kill', 'mad_protect'], nightAction: true, minPlayers: 8,
  },
  governor: {
    id: 'governor', arabicName: 'الحاكمة', team: 'citizens', type: 'special',
    image: 'assets/cards/new_roles/governor.png',
    description: 'كل ليلة تختار لاعبًا لتكشف كرته للجميع على الطاولة حتى نهاية اللعبة. لا تستطيع كشف المافيا الملعون.',
    color: '#c4b5fd', abilities: ['public_reveal'], nightAction: true, minPlayers: 8,
  },
};
...
const Effects = {
  ...
  init() {
    ...
    if (!this.isMobileLite()) this.spawnAsh();
  },

  spawnAsh() {
    if (this._isMobile || this.isMobileLite()) return;
  ...
  startMeteors() {
    ...
    const schedule = () => {
      this.spawnMeteor();
      const delay = (this._isMobile || this.isMobileLite())
        ? 12000 + Math.random() * 8000
        : 6000 + Math.random() * 6000;
      this._meteorTimeout = setTimeout(schedule, delay);
    };
    this._meteorTimeout = setTimeout(schedule, (this._isMobile || this.isMobileLite())
      ? 12000 + Math.random() * 8000
      : 6000 + Math.random() * 6000);
  },
  ...
  setNight() {
    this.startLightning();
    this.startMeteors();
    if (!this.isMobileLite()) this.startCrystalDust();
