...
const Effects = {
  ...
  startMeteors() {
    ...
    const schedule = () => {
      this.spawnMeteor();
      const delay = this.isMobileLite() ? 10000 + Math.random() * 14000 : 3500 + Math.random() * 6500;
      this._meteorTimeout = setTimeout(schedule, delay);
    };
    this._meteorTimeout = setTimeout(schedule, this.isMobileLite() ? 8000 + Math.random() * 8000 : 1200 + Math.random() * 3200);
  },
...
function getPlayerPortraitHtml(iconKey, alt = '', className = 'player-portrait') {
  ...
  return `<span class="${safeClass} animal-avatar" role="img" aria-label="${safeAlt}">${getAnimalEmoji(iconKey)}</span>`;
}
