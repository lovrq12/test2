...
const RoleEngine = {
  ...
  isPlayerNonMafiaForParity(player) {
    return !this.isPlayerMafia(player);
  },
  ...
  checkWin(players) {
    ...
    const aliveNonMafia = alive.filter(p => this.isPlayerNonMafiaForParity(p));

    if (aliveMafia.length > 0 && aliveMafia.length >= aliveNonMafia.length) return 'mafia';
    if (aliveMafia.length === 0 && aliveNonMafia.length > 0) return 'citizens';
    return null;
  },
};
