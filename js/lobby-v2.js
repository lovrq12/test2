...
(function () {
  ...
  function bindStarLifecycle() {
    const container = document.getElementById('lv2-stars');
    if (!container) return;
    document.addEventListener('visibilitychange', () => {
      if (document.hidden || isLobbyLowPower()) container.textContent = '';
      else initStars();
    });
    ...
    window.addEventListener('resize', () => {
      ...
      resizeTimer = setTimeout(() => {
        if (isLobbyLowPower()) container.textContent = '';
        else initStars();
      }, 150);
    });
    window.addEventListener('pagehide', () => { container.textContent = ''; });
  }
