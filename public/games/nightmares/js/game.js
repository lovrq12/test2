...
const CHEMIST_HEAL_USED_KEY = 'chemistHeal';
const CHEMIST_REVEAL_USED_KEY = 'chemistReveal';
const CHEMIST_SHIELD_USED_KEY = 'chemistShield';
const CHEMIST_POTION_ROUND_KEY = 'chemistPotionRound';
const BLACK_WIZARD_DISABLE_USED_KEY = 'blackWizardDisable';
const BLACK_WIZARD_LOCKED_TARGET_ROLES = ['doctor', 'detective', 'mad_citizen', 'governor', 'victim', 'poet', 'habait', 'chemist'];
...
function isCursedHiddenFromDetective() {
  return roomData?.settings?.cursedHiddenFromDetective === true;
}
...
function getDetectiveInvestigationResult(target = {}) {
  ...
  if (target.role === 'cursed' && isCursedHiddenFromDetective()) {
    return '✓ ليس مافيا';
  }
  if (target.role !== 'cursed' && isRoleHiddenFromInvestigation(target)) {
    return '✓ ليس مافيا';
  }
...
function isBlackWizardDisableActive(game = roomData?.game || {}) {
  return game?.phase === 'night'
    && Number(game.blackWizardDisabledRound || 0) === Number(game.round || 1);
}

function isNonMafiaAbilityLocked(player = {}, game = roomData?.game || {}) {
  return isBlackWizardDisableActive(game)
    && isPlayerActive(player)
    && !RoleEngine.isPlayerMafia(player);
}

function showLockedAbilityMessageIfNeeded(player = {}, game = roomData?.game || {}) {
  if (!isNonMafiaAbilityLocked(player, game)) return false;
  showToast('قدرتك غير متاحة هذه الليلة', 'info', 3000);
  return true;
}

function hasChemistUsedPotionThisNight(player = {}, game = roomData?.game || {}) {
  return Number(player?.usedAbilities?.[CHEMIST_POTION_ROUND_KEY] || 0) === Number(game?.round || 1);
}

function getChemistUsedPotionCount(player = {}) {
  const used = player?.usedAbilities || {};
  return [CHEMIST_HEAL_USED_KEY, CHEMIST_REVEAL_USED_KEY, CHEMIST_SHIELD_USED_KEY]
    .filter(key => !!used[key])
    .length;
}

function getChemistPotionBlockReason(player = {}, game = roomData?.game || {}, potionKey = '') {
  if (player?.usedAbilities?.[potionKey]) return 'تم استخدام هذه الجرعة';
  if (hasChemistUsedPotionThisNight(player, game)) return 'لا يمكنك استخدام أكثر من جرعة في نفس الليلة';
  return '';
}
...
function updateTimer(timerEndsAt) {
  ...
  function tick() {
    ...
    el.className = 'timer-display nm-hud-value' + (remaining <= 10 ? ' urgent' : '');
    const phaseHeader = document.getElementById('phase-header');
    if (phaseHeader) phaseHeader.classList.toggle('is-urgent', remaining <= 10);
...
function resolveNightActions(gameOrActions, players) {
  ...
  const round = game.round || 1;
  const blackWizardDisabled = isBlackWizardDisableActive(game);
  const chemistShieldActive = !blackWizardDisabled && Number(game.chemistShieldRound || 0) === Number(round);
  ...
  const doctorProtect = blackWizardDisabled ? null : actions.doctor_protect;
  const madAction = blackWizardDisabled ? null : (actions.madCitizen || null);
  ...
  if (mafiaKill) {
    ...
    if (isActiveAlive(target)) {
      ...
      if (chemistShieldActive || doctorSaved || madSaved) {
        protected_ = true;
      } else if (!blackWizardDisabled && target.role === 'immune_citizen' && !target.usedAbilities?.immune_used) {
        protected_ = true;
        immuneConsumed = mafiaKill;
      } else {
        killedIds.push(mafiaKill);
        if (!blackWizardDisabled && target.role === 'infected' && mafiaKillActor) {
...
function canUseChemistAbility(phase) {
  return phase === 'night';
}

function canUseBlackWizardAbility(phase) {
  return phase === 'night';
}
...
async function chooseDoctorProtect(targetPlayerId) {
  ...
  if (showLockedAbilityMessageIfNeeded(me, game)) return;
...
async function useChemistHeal(targetPlayerId) {
  const game = roomData?.game || {};
  const players = roomData?.players || {};
  const me = players[session.id];
  const target = players[targetPlayerId];
  if (!canUseChemistAbility(game.phase) || me?.role !== 'chemist' || !isActiveAlive(me)) return;
  if (!target || target.alive || !isPlayerActive(target) || targetPlayerId === session.id) return;
  if (showLockedAbilityMessageIfNeeded(me, game)) return;
  const blocked = getChemistPotionBlockReason(me, game, CHEMIST_HEAL_USED_KEY);
  if (blocked) {
    showToast(blocked, 'info', 3000);
    return;
  }
  if (RoleEngine.getPlayerTeam(target) !== 'citizens' || RoleEngine.isPlayerMafia(target)) {
    showToast('لا يمكن إحياء هذا اللاعب', 'error', 3500);
    return;
  }
  ...
  const updates = {
    [`rooms/${session.roomId}/players/${targetPlayerId}/alive`]: true,
    [`rooms/${session.roomId}/game/votes/${targetPlayerId}`]: null,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/${CHEMIST_HEAL_USED_KEY}`]: true,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/${CHEMIST_POTION_ROUND_KEY}`]: round,
    [`rooms/${session.roomId}/game/chemistRevives/${targetPlayerId}`]: {
      by: session.id,
      round,
      ts: Date.now(),
    },
  ...
  addGovernorReviveUnlockToUpdates(updates, targetPlayerId, target);
  addWinnerToUpdates(updates, { [targetPlayerId]: { alive: true } });
  await db.ref().update(updates);
  showCinematic('جرعة العلاج', `عاد ${target.name} للحياة.`, 3000);
}

async function useChemistReveal(targetPlayerId) {
  const game = roomData?.game || {};
  const players = roomData?.players || {};
  const me = players[session.id];
  const target = players[targetPlayerId];
  ...
  if (!isActiveAlive(target) || targetPlayerId === session.id) return;
  if (showLockedAbilityMessageIfNeeded(me, game)) return;
  const blocked = getChemistPotionBlockReason(me, game, CHEMIST_REVEAL_USED_KEY);
  if (blocked) {
    showToast(blocked, 'info', 3000);
    return;
  }
  ...
  Sound.playAbility();
  await db.ref().update({
    [`rooms/${session.roomId}/players/${session.id}/privateReveals/${targetPlayerId}`]: revealData,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/${CHEMIST_REVEAL_USED_KEY}`]: true,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/${CHEMIST_POTION_ROUND_KEY}`]: round,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/chemistRevealTargetId`]: targetPlayerId,
  });
  showWhisperReveal({ ...target, role: target.role }, 10, target.role);
  showToast(`كشفت هوية ${target.name}: ${RoleEngine.getRoleArabicName(target.role)}`, 'info', 4500);
}

async function useChemistShield() {
  const game = roomData?.game || {};
  const me = roomData?.players?.[session.id];
  if (!canUseChemistAbility(game.phase) || me?.role !== 'chemist' || !isActiveAlive(me)) return;
  if (showLockedAbilityMessageIfNeeded(me, game)) return;
  const blocked = getChemistPotionBlockReason(me, game, CHEMIST_SHIELD_USED_KEY);
  if (blocked) {
    showToast(blocked, 'info', 3000);
    return;
  }
  ...
  Sound.playAbility();
  await db.ref().update({
    [`rooms/${session.roomId}/game/chemistShieldRound`]: round,
    [`rooms/${session.roomId}/game/chemistShieldBy`]: session.id,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/${CHEMIST_SHIELD_USED_KEY}`]: true,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/${CHEMIST_POTION_ROUND_KEY}`]: round,
  });
  showToast('تم تفعيل جرعة الدرع لهذه الليلة', 'info', 3500);
}

async function useBlackWizardDisable() {
  const game = roomData?.game || {};
  const me = roomData?.players?.[session.id];
  if (!canUseBlackWizardAbility(game.phase) || me?.role !== 'blackWizard' || !isActiveAlive(me)) return;
  if (me.usedAbilities?.[BLACK_WIZARD_DISABLE_USED_KEY]) {
    showToast('تم استخدام التعطيل سابقًا', 'info', 3000);
    return;
  }
  ...
  Sound.playAbility();
  await db.ref().update({
    [`rooms/${session.roomId}/game/blackWizardDisabledRound`]: round,
    [`rooms/${session.roomId}/game/blackWizardDisabledBy`]: session.id,
    [`rooms/${session.roomId}/players/${session.id}/usedAbilities/${BLACK_WIZARD_DISABLE_USED_KEY}`]: true,
  });
  showToast('تم تعطيل قدرات غير المافيا هذه الليلة', 'info', 3500);
}
...
function handlePlayerTargetClick(targetPlayerId, anchorEl = null) {
  ...
  const canChemistHeal = me.role === 'chemist'
    && phase === 'night'
    && !me.usedAbilities?.[CHEMIST_HEAL_USED_KEY]
    && !hasChemistUsedPotionThisNight(me, game)
    && !targetIsAlive
    && !targetIsSelf;

  if (!targetIsAlive && !canPhoenixRevive && !canVictimRevive && !canChemistHeal) return;
  if (targetIsSelf && !(phase === 'night' && me.role === 'doctor') && !canPhoenixRevive) return;
  if (phase === 'night'
    && BLACK_WIZARD_LOCKED_TARGET_ROLES.includes(me.role)
    && isNonMafiaAbilityLocked(me, game)) {
    showLockedAbilityMessageIfNeeded(me, game);
    return;
  }
  ...
  if (phase === 'night' && me.role === 'chemist' && !targetIsSelf) {
    if (!targetIsAlive) {
      const healBlock = getChemistPotionBlockReason(me, game, CHEMIST_HEAL_USED_KEY);
      actions.push({
        label: healBlock || 'جرعة العلاج',
        disabled: !!healBlock,
        fn: () => useChemistHeal(targetPlayerId),
      });
    } else {
      const revealBlock = getChemistPotionBlockReason(me, game, CHEMIST_REVEAL_USED_KEY);
      actions.push({
        label: revealBlock || 'جرعة الكشف',
        disabled: !!revealBlock,
        fn: () => useChemistReveal(targetPlayerId),
      });
    }
  }
...
function getMafiaChoiceLabels(game, players, targetId, viewer) {
  ...
  const choices = game.nightActions?.mafiaKills || {};
  const mafiaOrder = Object.values(players || {})
    .filter(p => p && RoleEngine.isPlayerMafia(p) && isPlayerActive(p))
  ...
  return Object.entries(choices)
    ...
    .map(([mafiaId]) => {
      ...
      const orderIndex = Math.max(0, mafiaOrder.findIndex(p => p.id === mafiaId));
      const shortLabel = mafiaId === session.id ? 'اختيارك' : `مافيا ${orderIndex + 1}`;
      const selectorName = selector.name || `مافيا ${orderIndex + 1}`;
      return {
        label: shortLabel,
        title: mafiaId === session.id ? 'هذا اختيارك الحالي' : `${selectorName} اختار هذا الهدف`,
        mine: mafiaId === session.id,
      };
    })
    .filter(Boolean);
}
...

  {
    ...
    pArr.forEach((p, i) => {
      ...
      const canClick = (isActiveAlive(me) || phoenixCanActFromDeath) && isActive && (
        ...
        || (me?.role === 'chemist' && phase === 'night' && !me.usedAbilities?.[CHEMIST_HEAL_USED_KEY] && !hasChemistUsedPotionThisNight(me, game) && p.id !== session.id)
      );
      ...
      const isMyMafiaChoice = mafiaChoiceLabels.some(choice => choice.mine);
      ...
      seat.innerHTML = `
        ...
        <span class="ptc-frame">
          ...
          ${mafiaChoiceLabels.length ? `<span class="ptc-mafia-choice-stack">${mafiaChoiceLabels.map(choice => `<span class="ptc-mafia-choice-badge ptc-kill-badge${choice.mine ? ' my-selection' : ''}" title="${escapeHtml(choice.title)}">⚑ ${escapeHtml(choice.label)}</span>`).join('')}</span>` : ''}
        ...
        ${voteCount ? `<span class="ptc-vote-count-label">${voteCount} ${voteCount === 1 ? 'صوت' : 'أصوات'}</span>` : ''}
...
function renderBottomActionBar(game, me) {
  ...
  const titleEl = document.getElementById('bottom-vote-title');
  const selectedEl = document.getElementById('bottom-vote-selected');
  ...
  const targetName = currentVote === 'skip'
    ? 'تخطي التصويت'
    : currentVote
      ? (roomData?.players?.[currentVote]?.name || 'لاعب')
      : '';
  const tally = getVoteTally(game, roomData?.players || {});
  const totalVotes = Object.values(game.votes || {}).filter(Boolean).length + (game.founderVote ? 1 : 0);
  const targetVotes = currentVote && currentVote !== 'skip' ? (tally[currentVote] || 0) : 0;

  bar.classList.toggle('has-vote', !!currentVote);
  bar.classList.toggle('votes-frozen', !!votesFrozen);
  if (titleEl) titleEl.textContent = votesFrozen ? 'الأصوات مجمدة' : 'اختر لاعبًا للتصويت عليه';
  if (selectedEl) {
    selectedEl.textContent = votesFrozen
      ? 'قدرة المؤسس جمّدت التصويت لهذه المرحلة.'
      : currentVote
        ? `تم تسجيل اختيارك: ${targetName}${targetVotes ? ` — عليه ${targetVotes} ${targetVotes === 1 ? 'صوت' : 'أصوات'}` : ''}. إجمالي الأصوات: ${totalVotes}.`
        : `اضغط على لاعب من الطاولة ثم أكّد التصويت من النافذة. إجمالي الأصوات الآن: ${totalVotes}.`;
  }

  unvoteBtn.disabled = votesFrozen || !currentVote;
  unvoteBtn.classList.toggle('active', !!currentVote && currentVote !== 'skip');
  unvoteBtn.textContent = currentVote ? 'تغيير / سحب' : 'بدون اختيار';
...
function getRoleAbilityText(me = {}, roleData = {}) {
  ...
  const extraAbilityText = {
    ...
    chemist: 'ليلًا: اختر جرعة واحدة فقط. جرعة العلاج تحيي ميتًا من المواطنين، جرعة الكشف تكشف دور لاعب لك فقط، وجرعة الدرع تحمي الجميع من قتل المافيا تلك الليلة.',
    blackWizard: 'ليلًا: زر عطل يعطل قدرات غير المافيا لهذه الليلة فقط. مرة واحدة، ولا يوقف قتل المافيا.',
  };
...
function getRoleTimingText(me = {}, game = roomData?.game || {}) {
  ...
  if (['mafia', 'whisper', 'doctor', 'detective', 'poisoner', 'oathbreaker', 'mad_citizen', 'governor', 'victim', 'poet', 'habait', 'chemist', 'blackWizard'].includes(role)) {
    return 'الليل';
  }
...
function getRoleTargetTypeText(me = {}) {
  ...
  const targetMap = {
    ...
    poet: 'لاعب حي غيرك',
    habait: 'لاعب حي غيرك',
    chemist: 'العلاج: لاعب ميت من المواطنين، الكشف: لاعب حي، الدرع: بدون هدف',
    blackWizard: 'بدون هدف',
...
function getRoleCurrentTargetText(me = {}, game = roomData?.game || {}, players = roomData?.players || {}) {
  ...
  if (me.role === 'chemist') {
    if (Number(game.chemistShieldRound || 0) === Number(game.round || 1)
      && game.chemistShieldBy === session?.id) {
      return 'جرعة الدرع - هذه الليلة';
    }
    targetId = me.usedAbilities?.chemistRevealTargetId || null;
    detail = targetId ? ' - جرعة الكشف' : '';
  }
  if (me.role === 'blackWizard'
    && Number(game.blackWizardDisabledRound || 0) === Number(game.round || 1)
    && game.blackWizardDisabledBy === session?.id) {
    return 'تعطيل هذه الليلة';
  }
...
function getOnceAbilityKey(role) {
  const usedMap = {
    ...
    poet: 'poet_used',
    habait: 'habait_used',
    blackWizard: BLACK_WIZARD_DISABLE_USED_KEY,
  };
  return usedMap[role] || '';
}
...
function getRoleAbilityStatusText(me = {}, game = roomData?.game || {}) {
  ...
  if (isNonMafiaAbilityLocked(me, game)) return 'قدرتك غير متاحة هذه الليلة';
  if (me.role === 'chemist') {
    const remaining = 3 - getChemistUsedPotionCount(me);
    if (remaining <= 0) return 'تم استخدام كل الجرعات';
    if (hasChemistUsedPotionThisNight(me, game)) return 'استخدمت جرعة هذه الليلة';
  ...
  if (['mafia', 'whisper', 'doctor', 'detective', 'poisoner', 'oathbreaker', 'mad_citizen', 'governor', 'victim', 'poet', 'habait', 'blackWizard'].includes(me.role) || RoleEngine.isPlayerMafia(me)) {
    return phase === 'night' ? 'متاحة الآن' : 'تنتظر الليل';
  }
...
function getRoleUsesText(me = {}, game = roomData?.game || {}) {
  if (isNonMafiaAbilityLocked(me, game)) return 'مقفلة هذه الليلة';
  if (me.role === 'chemist') return `${Math.max(0, 3 - getChemistUsedPotionCount(me))} / 3 جرعات`;
...
function renderRoleConfirmAction(me = {}, game = roomData?.game || {}) {
  ...
  if (me.role === 'chemist'
    && isActiveAlive(me)
    && game.phase === 'night'
    && !me.usedAbilities?.[CHEMIST_SHIELD_USED_KEY]) {
    btn.hidden = false;
    btn.textContent = 'احمي';
    const blocked = isNonMafiaAbilityLocked(me, game) || hasChemistUsedPotionThisNight(me, game);
    btn.disabled = !!blocked;
    bindTouchSafeAction(btn, async () => {
      if (btn.disabled) {
        if (isNonMafiaAbilityLocked(me, game)) showLockedAbilityMessageIfNeeded(me, game);
        return;
      }
      btn.disabled = true;
      await useChemistShield();
    });
    return;
  }
  ...
  if (me.role === 'blackWizard'
    && isActiveAlive(me)
    && game.phase === 'night'
    && !me.usedAbilities?.[BLACK_WIZARD_DISABLE_USED_KEY]) {
    btn.hidden = false;
    btn.textContent = 'عطل';
    bindTouchSafeAction(btn, async () => {
      if (btn.disabled) return;
      btn.disabled = true;
      await useBlackWizardDisable();
    });
    return;
  }
...
function closeActionMenu() {
  ...
  if (menu) {
    ...
    menu.classList.remove('vote-action-sheet');
    menu.style.top = '';
    menu.style.left = '';
    menu.style.width = '';
  }
...
function showActionMenu(target, actions, anchorEl) {
  ...
  const game = roomData?.game || {};
  const isVotePhase = game.phase === 'voting' || game.phase === 'defense';
  ...
  if (isVotePhase) {
    ...
    hint.className = 'action-menu-hint';
    hint.textContent = 'هل تريد تسجيل هذا اللاعب كهدف للتصويت؟ يمكنك تغيير اختيارك قبل انتهاء الوقت.';
  ...
  actions.forEach(action => {
    ...
    if (isVotePhase && /تصويت/.test(action.label)) btn.classList.add('vote-confirm-btn');
...
function getWinnerLabel(winner) {
  if (winner === 'citizens') return { title: 'فوز المواطنين', subtitle: 'انتهت المباراة', className: 'citizens', team: 'المواطنون' };
  if (winner === 'mafia') return { title: 'فوز المافيا', subtitle: 'انتهت المباراة', className: 'mafia', team: 'المافيا' };
  if (winner === 'eclipse') return { title: 'فوز Eclipse', subtitle: 'انتهت المباراة', className: 'eclipse', team: 'Eclipse' };
...
function getWinPlayerTeamClass(player = {}) {
  const team = RoleEngine.getPlayerTeam(player);
  if (team === 'mafia') return 'mafia';
  if (team === 'neutral') return 'neutral';
  return 'citizens';
}

function sortWinPlayers(players = []) {
  return [...players].sort((a, b) => (a.seat ?? 99) - (b.seat ?? 99));
}

function getAliveGroupTitle(winner, isWinningGroup, count) {
  if (isWinningGroup) {
    if (winner === 'mafia') return `المافيا الناجون (${count})`;
    if (winner === 'citizens') return `المواطنون الناجون (${count})`;
    if (winner === 'eclipse') return `الفائز المنفرد (${count})`;
  }
  return count === 1 ? 'الناجي الآخر (1)' : `الأحياء الآخرون (${count})`;
}

function getWinDurationText(game = {}, settings = {}) {
  const startedAt = Number(game.startedAt || game.startTime || game.startedAtMs || 0);
  const elapsedSeconds = startedAt > 0 ? Math.floor((Date.now() - startedAt) / 1000) : 0;
  if (elapsedSeconds > 0 && elapsedSeconds < 60 * 60 * 24) return formatTime(elapsedSeconds);

  const durations = {
    night: Number(settings.nightTime || 60),
    morning: Number(settings.dayTime || 120),
    discussion: Number(settings.discussionTime || 90),
    voting: Number(settings.votingTime || 45),
    defense: Number(settings.defenseTime || 30),
  };
  const phaseOrder = ['night', 'morning', 'discussion', 'voting', 'defense'];
  const round = Math.max(1, Number(game.round || 1));
  const fullRoundSeconds = phaseOrder.reduce((sum, phase) => sum + durations[phase], 0);
  const phaseIndex = Math.max(0, phaseOrder.indexOf(game.phase));
  const currentRoundSeconds = phaseOrder
    .slice(0, phaseIndex + 1)
    .reduce((sum, phase) => sum + durations[phase], 0);
  return formatTime(Math.max(0, (round - 1) * fullRoundSeconds + currentRoundSeconds));
}

function renderWinPlayerCard(player, winner, includePendingWin, variant = 'alive') {
  ...
  const teamClass = getWinPlayerTeamClass(player);
  const stateClass = status === 'حي' ? 'alive' : status === 'ميت' ? 'dead' : 'removed';
  const selfLabel = player.id === session.id ? ' (أنت)' : '';

  return `
    <article class="win-player-card win-player-${variant} team-${teamClass} state-${stateClass} ${isWinner ? 'is-winner' : ''}">
      <span class="win-player-mark" aria-hidden="true">${variant === 'dead' ? '☠' : ''}</span>
      <img class="win-role-thumb" src="${roleData.image}" alt="${escapeHtml(roleData.arabicName)}" loading="lazy" decoding="async">
      <span class="win-player-copy">
        <strong>${escapeHtml(player.name || 'لاعب')}${selfLabel}</strong>
        <small style="color:${roleData.color || '#d8b4fe'}">${escapeHtml(roleData.arabicName)}</small>
      </span>
      <span class="win-state-chip">${status}</span>
      <span class="win-stat-mini">فوز ${stats.totalWins}</span>
    </article>
  `;
}

function renderWinAliveGroup(title, players, winner, includePendingWin, tone) {
  const content = players.length
    ? players.map(player => renderWinPlayerCard(player, winner, includePendingWin, 'alive')).join('')
    : '<div class="win-empty-row">لا يوجد لاعبون أحياء في هذه المجموعة</div>';
  return `
    <div class="win-alive-group tone-${tone}">
      <h3>${escapeHtml(title)}</h3>
      <div class="win-alive-list">${content}</div>
    </div>
  `;
}

function renderWinSummary(label, winner, players, game, settings) {
  return `
    <section class="win-match-summary" aria-label="ملخص المباراة">
      <div class="win-summary-item">
        <span class="win-summary-icon" aria-hidden="true">⚔</span>
        <span>إجمالي الجولات</span>
        <strong>${Number(game.round || 1)}</strong>
      </div>
      <div class="win-summary-item">
        <span class="win-summary-icon" aria-hidden="true">⌛</span>
        <span>مدة المباراة</span>
        <strong>${getWinDurationText(game, settings)}</strong>
      </div>
      <div class="win-summary-item winner-${escapeHtml(label.className)}">
        <span class="win-summary-icon" aria-hidden="true">♛</span>
        <span>الفريق الفائز</span>
        <strong>${escapeHtml(label.team)}</strong>
      </div>
    </section>
  `;
}
...
function showWinScreen(winner, players) {
  ...
  const game = roomData?.game || {};
  const settings = roomData?.settings || {};
  const allPlayers = sortWinPlayers(Object.entries(players || {})
    .map(([id, player]) => ({ ...player, id: player.id || id })));
  const alivePlayers = allPlayers.filter(player => isActiveAlive(player));
  const outPlayers = allPlayers.filter(player => !isActiveAlive(player));
  const winningAlivePlayers = winner === 'eclipse'
    ? allPlayers.filter(player => player.role === 'eclipse')
    : alivePlayers.filter(player => isPlayerOnWinningSide(player, winner));
  const otherAlivePlayers = alivePlayers.filter(player => !winningAlivePlayers.some(winnerPlayer => winnerPlayer.id === player.id));
  const deadList = outPlayers.length
    ? outPlayers.map(player => renderWinPlayerCard(player, winner, includePendingWin, 'dead')).join('')
    : '<div class="win-empty-row">لا يوجد موتى</div>';

  title.textContent = label.title;
  title.className = `win-title ${label.className}`;
  sub.textContent = label.subtitle;
  if (ws) {
    ws.dataset.winner = label.className || 'unknown';
    ws.setAttribute('aria-hidden', 'false');
  }

  rolesDiv.innerHTML = `
    <section class="win-panel win-panel-alive" aria-label="الأحياء">
      <h2 class="win-panel-title">الأحياء</h2>
      <div class="win-alive-groups">
        ${renderWinAliveGroup(getAliveGroupTitle(winner, true, winningAlivePlayers.length), winningAlivePlayers, winner, includePendingWin, 'winner')}
        ${renderWinAliveGroup(getAliveGroupTitle(winner, false, otherAlivePlayers.length), otherAlivePlayers, winner, includePendingWin, 'other')}
      </div>
    </section>
    <section class="win-panel win-panel-dead" aria-label="الموتى">
      <h2 class="win-panel-title">الموتى</h2>
      <div class="win-dead-list">${deadList}</div>
    </section>
    ${renderWinSummary(label, winner, allPlayers, game, settings)}
  `;
